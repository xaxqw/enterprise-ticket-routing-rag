...
# ---------------- 2. 技术架构 ----------------
h1("二、技术架构总览（端到端）")
para("数据链路：文档(多格式) → 解析清洗 → 语义分块 → 向量化(FAISS/nomic-embed-text) + BM25 双索引 → 混合检索 → 本地 RRF 重排 → 多智能体编排 → LLM 生成 → 溯源/幻觉校验 → 返回。")
bullet("接入层：FastAPI（8000，JWT 多租户鉴权）+ Streamlit 问答界面（8501）。")
bullet("检索层：BM25 关键词检索（jieba 分词）+ 向量检索（本机 Ollama nomic-embed-text，768 维，多语言/中文 SOTA），加权融合后经本地 RRF（Reciprocal Rank Fusion）重排——零模型依赖、完全免费离线。")
...
bullet("生成层：三 LLM 后端——本地 Ollama（deepseek-r1，默认免费/离线/调用本机 GPU）+ 在线 SiliconFlow（Qwen2.5 系列，可选）+ 本地 LoRA（Qwen2.5-0.5B + LoRA，可选私有化），按 LLM_BACKEND 切换。")
...
bullet("本地 deepseek-r1 偶发把 2018 写成 2218、62% 写成 61%；生成后保留数字落地校验：抽取答案中的年份/百分比/整数，"
...
h2("3.8 三 LLM 后端与本地化")
bullet("本地 Ollama（默认）：deepseek-r1 由本机 Ollama 运行，自动调用 RTX 4050 GPU，完全免费、离线、零 API Key，是本系统演示与交付的默认形态。")
...
qa("Embedding 模型怎么选？为什么要 L2 归一化？",
   "用本机 Ollama 的 nomic-embed-text（768 维，多语言/中文 SOTA，免费离线）。L2 归一化后向量内积等于余弦相似度，"
...
qa("大模型后端怎么设计？本地和在线怎么切换？",
   "统一生成入口按 LLM_BACKEND 切换：默认走本地 Ollama（deepseek-r1，免费/离线/GPU），无网也能跑；"
...
qa("有什么不足 / 后续规划？（诚实回答）",
   "① 默认本地 deepseek-r1（免费/离线/GPU），数字精度由校验层兜底，生产可换更大本地模型（如 qwen2.5:14b）或做抽取式约束；"
...
bullet("若被问‘本地 LoRA 真的训过吗’：可说本地推理链路（bitsandbytes 4bit）已打通，LoRA 适配器是可选私有化后端，"
     "主链路默认走本机 Ollama deepseek-r1 保证演示稳定——不要夸大没训过的部分。")
