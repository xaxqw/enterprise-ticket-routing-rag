...
| **拒答保护（相关性闸门）** | 检索不到相关内容时直接拒答（不调用大模型编造），避免幻觉；向量余弦 + BM25 双阈值可配 |
...
系统内置**相关性闸门**：当知识库中检索不到与问题相关的内容时，直接返回「未检索到相关内容，无法回答」，**不会调用大模型硬答 / 编造**，避免幻觉式胡说。

- 判定使用**原始分数**（不受 RRF 排名融合与按查询 min-max 归一化的影响）：
  - 向量检索返回原始余弦相似度（`nomic-embed-text` 下相关文本对通常 >0.4，无关对约 0.1~0.2）；
  - BM25 返回原始关键词分数（精确命中 >0）。
- 二者**任一达标**即视为「有匹配」；仅当「两者都未达标」或「无任何检索结果」时才拒答。
- 阈值可在 `.env` 调整：`RELEVANCE_VECTOR_THRESHOLD`（默认 0.30）、`RELEVANCE_BM25_THRESHOLD`（默认 1.0）。调小更宽松，调大更严格。
- 前端对拒答以醒目告警样式展示，问答历史也会标记为「未匹配」便于回看。

> 典型场景：证书等文档尚未上传 / 未成功入库时，询问该证书内容会被正确拒答，而不是拿其他文档编造答案。
